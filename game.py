...
class GameStateData:
    ...
    def initialize( self, layout, numGhostAgents, random_start_pos = False):
        """
        ...
        for isPacman, pos in layout.agentPositions:
            ...
            if random_start_pos:
                ...
                while pos in list(map(lambda x: x.configuration.pos, self.agentStates)):
                    pos = layout.getRandomLegalPosition()
                ...
                if self.catchExceptions:
                    try:
                        ...
                        try:
                            ...
                            print("Agent %d timed out on observation!")
                            skip_action = True
            ...
            if self.catchExceptions:
                try:
                    ...
                    if self.totalAgentTimes[agentIndex] > self.rules.getMaxTotalTime(agentIndex):
                        ...
                        print("Used %d on this move, and %d so far this turn" % (
                            move_time, self.totalAgentTimes[agentIndex]), file=sys.stderr)
                        self.agentTimeout = True
                        self._agentCrash(agentIndex, quiet=True)
                        self.unmute()
                        return
